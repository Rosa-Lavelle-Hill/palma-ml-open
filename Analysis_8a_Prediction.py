...
from RUN_Analysis_8 import start_string, global_seed, t, analysis_path, use_preoptimised_model
from Fixed_params import cat_vars_agg, cat_vars, drop_list
...
if use_preoptimised_model == False:
    ...
    for file in os.listdir(directory_bits):
        ...
        if filename.endswith("_preprocessed.csv"):
            ...
            df_enet = dummy_code(df=df, vars_to_code=cat_vars_agg)
            ...
            param_df.to_csv(analysis_path +'Outputs/Modelling/Grid_Search/Enet/' + 'df{}_{}{}.csv'.format(df_num, start_string, t))
            ...
            grid_search.fit(X_train, y_train)
            param_df = pd.DataFrame(grid_search.cv_results_).sort_values(by='rank_test_score')
            param_df.to_csv(analysis_path +'Outputs/Modelling/Grid_Search/RF/'+'df{}{}.csv'.format(df_num, t))
