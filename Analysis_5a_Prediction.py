...
from RUN_Analysis_5 import start_string, global_seed, rf_params, enet_params, analysis_path, data_path, version
...
for file in os.listdir(directory_bits):
    ...
    if filename.endswith("_preprocessed{}.csv".format(version)):
        ...
        grid_search.fit(X_train, y_train)
        param_df = pd.DataFrame(grid_search.cv_results_).sort_values(by='rank_test_score')
        param_df.to_csv(analysis_path +'Outputs/Modelling/Grid_Search/RF/'+'df{}.csv'.format(df_num))
